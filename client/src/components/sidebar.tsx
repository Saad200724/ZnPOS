import { Link, useLocation } from "wouter";
import { useAuth } from "@/lib/auth";
import { Button } from "@/components/ui/button";
import { 
  LayoutDashboard, 
  ShoppingCart, 
  Package, 
  Users, 
  UserCheck, 
  BarChart3, 
  Settings, 
  LogOut 
} from "lucide-react";
import znforgeLogo from "@assets/ZnForge_Logo_1757783430022.png";
import meowLogo from "@assets/logo_1757859161982.png";

const navigation = [
  { name: "Dashboard", href: "/", icon: LayoutDashboard, permission: null },
  { name: "POS", href: "/pos", icon: ShoppingCart, permission: "pos" as const },
  { name: "Inventory", href: "/inventory", icon: Package, permission: "inventory" as const },
  { name: "Customers", href: "/customers", icon: Users, permission: "customers" as const },
  { name: "Staff", href: "/employees", icon: UserCheck, permission: "employees" as const },
  { name: "Reports", href: "/reports", icon: BarChart3, permission: "reports" as const },
  { name: "Settings", href: "/settings", icon: Settings, permission: "settings" as const },
];

export default function Sidebar() {
  const [location] = useLocation();
  const { user, business, logout } = useAuth();

  return (
    <div className="w-56 emerald-gradient text-white flex-shrink-0 flex flex-col h-screen">
      {/* Header */}
      <div className="p-4 border-b border-emerald-400">
        <div className="flex items-center space-x-3">
          <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center p-1">
            <img src={znforgeLogo} alt="ZnPOS Logo" className="w-full h-full object-contain" />
          </div>
          <div>
            <h1 className="text-xl font-bold">ZnPOS</h1>
            <p className="text-emerald-200 text-sm font-medium">Meow Meow Pet Shop</p>
          </div>
        </div>
      </div>
      
      {/* Navigation */}
      <nav className="flex-1 mt-4">
        <div className="px-3 space-y-1">
          {navigation
            .filter((item) => {
              // Show dashboard to everyone
              if (!item.permission) return true;
              // Show all to admin users
              if (user?.role === 'admin') return true;
              // For non-admin users, check their specific permissions
              return user?.permissions?.[item.permission] === true;
            })
            .map((item) => {
              const Icon = item.icon;
              const isActive = location === item.href || 
                (item.href === "/" && location === "/");
              
              return (
                <Link key={item.name} href={item.href}>
                  <div className={`sidebar-item ${isActive ? 'active' : ''}`}>
                    <Icon className="w-4 h-4 mr-2" />
                    {item.name}
                  </div>
                </Link>
              );
            })}
        </div>
      </nav>
      
      {/* User Profile - Fixed at bottom */}
      <div className="p-3 mt-auto bg-emerald-600">
        <div className="bg-emerald-700 rounded-lg p-3 mb-3">
          <div className="flex items-center space-x-3">
            <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center overflow-hidden">
              <img src={meowLogo} alt="Meow Meow Pet Shop" className="w-full h-full object-cover" />
            </div>
            <div>
              <p className="font-medium">{user?.username || 'User'}</p>
              <p className="text-emerald-200 text-sm capitalize">{user?.role || 'employee'}</p>
            </div>
          </div>
        </div>
        <Button 
          onClick={() => logout()} 
          variant="ghost" 
          className="w-full text-emerald-100 hover:bg-emerald-700"
        >
          <LogOut className="w-4 h-4 mr-2" />
          Sign Out
        </Button>
      </div>
    </div>
  );
}
